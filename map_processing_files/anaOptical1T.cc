...
int main(){
  ...
  TString fileName = TString("LAr_PEN_")+to_string(time.GetTime())+TString("-")+to_string(time.GetDate())+TString(".root");
...
//  TFile *outFileOpticalMap = new TFile(TString("OpticalMap.")+to_string(time.GetTime())+TString("-")+to_string(time.GetDate())+TString(".root"),"recreate");
  ...
  Double_t maxX = 1500.,minX = -1500.,maxY= 1500.,minY = -1500,maxZ= 1000,minZ = -1000,minR = 0, maxR = 3500.;
  ...
  //int Z = 0;
  int PENPartsZ[] = {125,154,182,210,211,239,267,296};
  for(int k =208; k <221; k++){ //TODO change the limits of k when merging PEN maps //for fibers make it 39 to 382
    //TODO--Uncomment the next line as well when merging PEN maps
    int Z = PENPartsZ[k]; 
    //TODO--change input dir
    TString dir = "";
	//dir ="/home/rushabh_gala/scripts/";
    dir = "/home/rushabh_gala/LGND1000_14String_Sims/OpticalSims/LAr_SiPlates_sept30_actuallyPEN/";
    //dir ="/home/rushabh_gala/LGND1000_14String_Sims/OpticalSims/PENPENPlatesPhysDetNoOverlap1000kg/"; //for PEN Plates TODO
	//TODO -- change fileName
    //
    TString fileName = "LAr_SiPlates_sept30Interior_" + to_string(k);
    //TString fileName = "PENPENPlatesPhysDetNoOverlap1000kgInterior_" + to_string(Z); //TODO replace k with Z for PEN map
    ...
    for(Int_t i = 0; i < nentries ; i++){
      ...
      */
      ...
      for (Int_t j = 0; j < eventSteps->GetNSteps();j++){
        ...
        if((physName.Contains("SiPM")||physName.Contains("CanShroud")||physName.Contains("Cryostat")) && step->GetEdep() > 0 && pastTrackID != trackID){
