...
//  TFile *outFileOpticalMap = new TFile(TString("OpticalMap.")+to_string(time.GetTime())+TString("-")+to_string(time.GetDate())+TString(".root"),"recreate");
  ...
  Double_t maxX = 1000.,minX = -1000.,maxY=1000.,minY = -1000,maxZ= 850,minZ = -900,minR = 0, maxR = 1000.;
  ...
  TH3D* hMap = new TH3D("ProbMapInterior","ProbMapInterior",nbinsX,minX,maxX,nbinsY,minY,maxY,nbinsZ,minZ,maxZ);
  TH3D* hMapRaw = new TH3D("ProbMapInterior_Raw","ProbMapInterior_Raw",nbinsX,minX,maxX,nbinsY,minY,maxY,nbinsZ,minZ,maxZ);
  TH3D* hMapScaled = new TH3D("ProbMapInterior_Scaled","ProbMapInterior_Scaled",nbinsX,minX,maxX,nbinsY,minY,maxY,nbinsZ,minZ,maxZ);

  TH2D* h2DMapRZ = new TH2D("LAr_OpticalMap_RZ","LAr_OpticalMap_RZ",nbinsR,minR,maxR,nbinsZ,minZ,maxZ);
  TH2D* h2DMapRZRaw = new TH2D("LAr_OpticalMap_RZRaw","LAr_OpticalMap_RZRaw",nbinsR,minR,maxR,nbinsZ,minZ,maxZ);
  TH2D* h2DMapRZScaled = new TH2D("LAr_OpticalMap_RZScaled","LAr_OpticalMap_RZScaled",nbinsR,minR,maxR,nbinsZ,minZ,maxZ);

  TH2D* h2DMapXY = new TH2D("LAr_OpticalMap_XY","LAr_OpticalMap_XY",nbinsX,minX,maxX,nbinsY,minY,maxY);
  TH2D* h2DMapXYRaw = new TH2D("LAr_OpticalMap_XYRaw","LAr_OpticalMap_XYRaw",nbinsX,minX,maxX,nbinsY,minY,maxY);
  TH2D* h2DMapXYScaled = new TH2D("LAr_OpticalMap_XYScaled","LAr_OpticalMap_XYScaled",nbinsX,minX,maxX,nbinsY,minY,maxY);

  TH2D* h2DMapYZ = new TH2D("LAr_OpticalMap_YZ","LAr_OpticalMap_YZ",nbinsY,minY,maxY,nbinsZ,minZ,maxZ);
  TH2D* h2DMapYZRaw = new TH2D("LAr_OpticalMap_YZRaw","LAr_OpticalMap_YZRaw",nbinsY,minY,maxY,nbinsZ,minZ,maxZ);  
  TH2D* h2DMapYZScaled = new TH2D("LAr_OpticalMap_YZScaled","LAr_OpticalMap_YZScaled",nbinsY,minY,maxY,nbinsZ,minZ,maxZ);  
  ...
  int Z = 0;
  int PENPartsZ[] = {97,98,116,117,135,136,154,155,173,174,192,193,211,212,230,231};
  for(int k =190; k <=240; k++){ //TODO change the limits of k when merging PEN maps
    ...
    //dir = "/data4/legend/sim/l200sims/LGND200_14String_Optical/fibers_realistic/";
    //dir = "/home/rushabh_gala/LGND200_14String_PENSims/OpticalSims/PENPENPlateModified/";
    //dir ="/home/rushabh_gala/OpticalSims/PENSimsPEN/"; // uncomment for PEN maps
    //dir ="/home/rushabh_gala/OpticalSims/LArSimsPEN/";
    dir ="/home/rushabh_gala/LGND1000_14String_Sims/OpticalSims/LArSiPlates/";
    //TODO -- change fileName
    //TString fileName = "optical14SiPMArray" + to_string(k);
    //TString fileName = "PEN_opticalInterior_" + to_string(k);
    //TString fileName = "PENGen_" + to_string(k);
    //TString fileName = "PENPENPlateModifiedInterior_" + to_string(Z); //TODO uncomment for PEN maps
    //TString fileName = "PENInterior_" + to_string(k);
    TString fileName = "LArSiPlatesInterior_" + to_string(k); //LGND 1000
