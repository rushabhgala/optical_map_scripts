...
int main(){
  ...
  TString fileName = TString("L200_PEN_Inner")+to_string(time.GetTime())+TString("-")+to_string(time.GetDate())+TString(".root");
...
//  TFile *outFileOpticalMap = new TFile(TString("OpticalMap.")+to_string(time.GetTime())+TString("-")+to_string(time.GetDate())+TString(".root"),"recreate");
  ...
  TNtuple *ntpleStepSiPM = new TNtuple("Optical_LAr","Optical_LAr","event:edep:ke:x:y:z:r:primX:primY:primZ:primR:primTheta");
  ...
  Double_t maxX = 1500.,minX = -1500.,maxY=1500.,minY = -1500.,maxZ= 3000.,minZ = -3000,minR = 0, maxR = 2750.;
  ...
  TH2D* h2DMapRZ = new TH2D("PEN_OpticalMap_RZ","PEN_OpticalMap_RZ",nbinsR,minR,maxR,nbinsZ,minZ,maxZ);
  TH2D* h2DMapRZRaw = new TH2D("PEN_OpticalMap_RZRaw","PEN_OpticalMap_RZRaw",nbinsR,minR,maxR,nbinsZ,minZ,maxZ);
  TH2D* h2DMapRZScaled = new TH2D("PEN_OpticalMap_RZScaled","PEN_OpticalMap_RZScaled",nbinsR,minR,maxR,nbinsZ,minZ,maxZ);

  TH2D* h2DMapXY = new TH2D("PEN_OpticalMap_XY","PEN_OpticalMap_XY",nbinsX,minX,maxX,nbinsY,minY,maxY);
  TH2D* h2DMapXYRaw = new TH2D("PEN_OpticalMap_XYRaw","PEN_OpticalMap_XYRaw",nbinsX,minX,maxX,nbinsY,minY,maxY);
  TH2D* h2DMapXYScaled = new TH2D("PEN_OpticalMap_XYScaled","PEN_OpticalMap_XYScaled",nbinsX,minX,maxX,nbinsY,minY,maxY);

  TH2D* h2DMapYZ = new TH2D("PEN_OpticalMap_YZ","PEN_OpticalMap_YZ",nbinsY,minY,maxY,nbinsZ,minZ,maxZ);
  TH2D* h2DMapYZRaw = new TH2D("PEN_OpticalMap_YZRaw","PEN_OpticalMap_YZRaw",nbinsY,minY,maxY,nbinsZ,minZ,maxZ);  
  TH2D* h2DMapYZScaled = new TH2D("PEN_OpticalMap_YZScaled","PEN_OpticalMap_YZScaled",nbinsY,minY,maxY,nbinsZ,minZ,maxZ);  
  ...
  for(int k =300; k <=400; k++){ //TODO change the limits of k when merging PEN maps
    //TODO--Uncomment the next line as well when merging PEN maps
    //TODO--change input dir
    TString dir = "";
    dir ="/data9/legend/LGND_1T_CDR_LNGS_Sims/OpticalSims/slab/lar-cryostat/";
    //TODO -- change fileName
    TString fileName = "LGND_1T_CDR_LGNDLiquidArgonLNGSExternal_" + to_string(k); //LGND 1000
    cout << k << endl;
